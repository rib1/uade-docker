...
- **Hadolint Code Scanning:** Do not upload clean Hadolint SARIF files with an empty `results` array just to refresh GitHub Code Scanning timestamps. In this repo, `github/codeql-action/upload-sarif` can fail with a misleading token/permission error for empty Hadolint SARIF. Keep the empty-result guard and rely on the Docker quality flow for clean Hadolint validation.
