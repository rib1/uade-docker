...
- **Branch Protection Gates:** For required test gates with path-sensitive behavior, keep the workflow trigger broad and do change detection inside the workflow. Use an always-present final gate job so branch protection has a stable check to require even when the expensive test job is intentionally skipped.
...
- **Version Pins:** Keep benchmark-tool version pins in the same `/test` manifest surfaces used by the rest of the repo. In this repo, `test/docker-compose.tooling.yml` is the right home for the `k6` image tag, even when the benchmark runner installs the binary into its own custom image. A k6 tag bump also requires the release archive checksum and `K6_CHECKSUM_VERSION` in `test/Dockerfile.benchmark` to change together; enforce the version alignment in the normal quality flow because Dependabot cannot update the checksum.
...
- **Alpine Tooling Images:** Quality and endpoint helper images can drift when their upstream Alpine-based bases move, even if Dependabot only changed a different test image. If `node:*-alpine`, `alpine:latest`, or `alpine:<version>` starts resolving newer package families, update all affected `apk add` pins together and rerun the Docker quality flow plus the touched runner build.
- **Alpine Pin Automation:** Use `node test/check-alpine-apk-pins.mjs --fix` to refresh stale Alpine `apk add` version-family pins. CI runs the same checker before Docker Compose builds so fresh runner package indexes fail with a precise updater command instead of a generic BuildKit solve error.
- **Hadolint Code Scanning:** `github/codeql-action/upload-sarif` reserves filenames ending in `.quality.sarif` for GitHub Code Quality. A generated name such as `hadolint-Dockerfile.quality.sarif` is routed to the wrong API and fails with a misleading token/permission error even when `security-events: write` is present. Append a neutral suffix such as `-results.sarif`, and keep uploading valid empty analyses so stale per-category alerts can close. Analysis categories are identity keys too, so preserve the historical root category `hadolint`; changing it to `hadolint-Dockerfile` leaves old alerts permanently open even after a clean scan.
- **Filesystem Pins:** `gcsfs==2026.5.0` rejects `fsspec==2026.4.0` because it requires `fsspec!=2026.4.0,>=2026.3.0`, while `s3fs==2026.4.0` requires `fsspec==2026.4.0`. Pinning `fsspec==2026.5.0`, `gcsfs==2026.5.0`, and `s3fs==2026.5.0` also fails as of 2026-05-25 because `fsspec==2026.5.0` and `s3fs==2026.5.0` are not published. Hold the filesystem trio at `2026.4.0` until a compatible release set is available.
...
- **Version Sync:** Keep `test/docker-compose.tooling.yml` as the single Hadolint image source for local quality and GitHub Security, and monitor `/test` with Dependabot's `docker-compose` ecosystem. The security workflow should resolve and run that image directly instead of using a separately versioned Hadolint action, and it must upload valid empty SARIF results so resolved alerts are closed.
...
- **Debian Runtime Pins:** For Debian `apt-get install`, tilde constraints like `curl=~8.14` do not mean "latest patch in this family" and fail resolution. Wildcard pins such as `curl=8.14.1-2+deb13u*` and `rsync=3.4.1+ds1-5+deb13u*` keep Hadolint satisfied while allowing Debian security revision bumps.
